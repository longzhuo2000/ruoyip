  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="规格" prop="specifications">
        <el-select v-model="queryParams.specifications" placeholder="请选择规格" clearable size="small">
          <el-option
            v-for="dict in dict.type.drugs_storage_specifications"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="单位" prop="unit">
        <el-select v-model="queryParams.unit" placeholder="请选择单位" clearable size="small">
          <el-option
            v-for="dict in dict.type.drugs_storage_company"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="infoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="规格" align="center" prop="specifications">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.drugs_storage_specifications" :value="scope.row.specifications"/>
        </template>
      </el-table-column>
      <el-table-column label="单位" align="center" prop="unit">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.drugs_storage_company" :value="scope.row.unit"/>
        </template>
      </el-table-column>
...
<script>
import { listInfo, getInfo, delInfo, addInfo, updateInfo, exportInfo } from "@/api/drug/info";

export default {
  name: "Info",
  dicts: ['drugs_storage_specifications', 'drugs_storage_company'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有药品数据项？').then(() => {

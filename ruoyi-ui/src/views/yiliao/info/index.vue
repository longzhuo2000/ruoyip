<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="1启用2挂失3作废4退卡" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择1启用2挂失3作废4退卡" clearable size="small">
          <el-option
            v-for="dict in dict.type.cost_outpatient_info"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="infoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="1启用2挂失3作废4退卡" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.cost_outpatient_info" :value="scope.row.status"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="1启用2挂失3作废4退卡" prop="status">
          <el-select v-model="form.status" placeholder="请选择1启用2挂失3作废4退卡">
            <el-option
              v-for="dict in dict.type.cost_outpatient_info"
              :key="dict.value"
              :label="dict.label"
...
<script>
import { listInfo, getInfo, delInfo, addInfo, updateInfo, exportInfo } from "@/api/yiliao/info";

export default {
  name: "Info",
  dicts: ['cost_outpatient_info'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有门诊卡信息数据项？').then(() => {
